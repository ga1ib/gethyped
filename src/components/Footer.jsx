...
function GHStamp() {
  ...
  return (
    <div className="relative w-[80px] h-[80px] sm:w-[100px] sm:h-[100px] md:w-[110px] md:h-[110px] lg:w-[120px] lg:h-[120px] select-none">
      <svg viewBox="0 0 108 108" className="absolute inset-0 w-full h-full
           animate-[spin_18s_linear_infinite]">
        <circle cx="54" cy="54" r="52" fill="#FCB8FA" />
        <defs>
          <path id="gh-ring"
            d="M54,54 m-40,0 a40,40 0 1,1 80,0 a40,40 0 1,1 -80,0" />
        </defs>
        <text style={{ fontSize:'clamp(7px, 2vw, 9px)', fontWeight:'800', fill:'#111', letterSpacing:'0.4px' }}>
          <textPath href="#gh-ring">{STAMP_TEXT}</textPath>
        </text>
      </svg>
      {/* Centre GH — does NOT rotate */}
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="font-black text-[clamp(1rem,4vw,1.6rem)] text-black tracking-tight leading-none">GH</span>
      </div>
    </div>
  )
}
...
export default function Footer() {
  return (
    <footer id="contact" className="bg-gray-100">
      {/* Big heading */}
      <div className="text-center px-4 sm:px-5 pt-8 sm:pt-12 md:pt-16">
        <h2 className="text-[clamp(2rem,8vw,4rem)] sm:text-[clamp(2.5rem,9vw,5rem)] md:text-[clamp(3rem,9vw,6rem)] font-black tracking-[-0.03em] text-black">
          Let&apos;s Get Hyped!
        </h2>

        {/* Two CTA buttons */}
        <div className="flex flex-wrap gap-8 sm:gap-6 justify-center mt-6 sm:mt-8 md:mt-10 pb-10 sm:pb-12 md:pb-16">
          <a
            href="mailto:info@gethyped.nl"
            className="inline-flex items-center border border-black rounded-lg overflow-hidden
                      font-semibold text-sm sm:text-md cursor-pointer hover:shadow-lg transition-shadow duration-200"
          >
            <span className="px-4 sm:px-6 py-2.5 sm:py-3.5 bg-white text-lg text-black leading-none whitespace-nowrap">Mail ons direct</span>
          ...
          <a
            href="#contact"
            className="inline-flex items-center rounded-lg overflow-hidden font-semibold text-lg sm:text-lg
                       bg-[#FF5C00] cursor-pointer hover:shadow-lg transition-shadow duration-200"
          >
            <span className="px-4 sm:px-6 py-2.5 sm:py-3.5 text-white leading-none whitespace-nowrap">Get Results</span>
  ...
  <div className="rounded-[1.5rem] sm:rounded-[2rem] overflow-hidden">
    ...
    <div
      className="
        ...
        [clip-path:polygon(0%_100%,0%_20%,100%_0%,100%_100%)] md:[clip-path:polygon(0%_100%,0%_30%,100%_0%,100%_100%)] lg:[clip-path:polygon(0%_100%,0%_80%,100_0%,100%,0%,100%_100%)]"
    >
           {/* g:[clip-path:polygon(0%_100%,0%_80%,100%_0%,100%_100%)] */}
          <div className="pr-4 sm:pr-6 md:pr-10 py-8">
            {/* MAIN GRID from first snippet */}
            <div className="grid gap-6 sm:gap-8 grid-cols-3 lg:grid-cols-3">
              
              {/* LEFT (LOGO) */}
              <div className="col-span-2 lg:col-span-1 -mt-1 md:mt-4 lg:mt-2 flex flex-col gap-6">
                {/* Logo */}
                <a href="#" className="inline-block mt-5 sm:mt-10">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 195 84"
                       width="clamp(100px, 22vw, 180px)" height="auto" aria-label="Get Hyped">
                ...
                {/* NAV + SOCIALS (ONLY visible on md & sm) */}
                <div className="flex flex-col gap-5 lg:hidden">
                  {/* Nav */}
                  <nav className="flex flex-wrap md:ml-5 ml-3 md:gap-2 gap-2 ">
                    {NAV_LINKS.map(({ label, href }) => (
                      <a
                        key={label}
                        href={href}
                        className="relative px-1 py-1 md:px-1 md:py-1.5 bg-white rounded-lg font-bold text-sm sm:text-xs
                                   overflow-hidden group transition hover:scale-[1.02]"
                      >
              ...
              <div className="hidden lg:flex flex-col gap-6 justify-center">
                {/* Nav */}
                <nav className="flex flex-wrap sm:ml-5 gap-3">
                  {NAV_LINKS.map(({ label, href }) => (
                    <a
                      key={label}
                      href={href}
                      className="relative   px-3 py-2 bg-white rounded-lg  font-bold text-sm
                                 overflow-hidden group transition hover:scale-[1.02]"
                    >
                      <span className="absolute inset-0 bg-orange-500 translate-y-full group-hover:translate-y-0 transition duration-200 rounded-lg" />
                      <span className="absolute inset-0 bg-black translate-y-full group-hover:translate-y-0 transition duration-300 delay-100 rounded-lg" />
              ...
              {/* RIGHT (CONTACT) */}
              <div className="col-span-1 flex flex-col gap-5 items-start sm:items-end sm:text-right">
                <div>
        ...
        {/* GH stamp */}
        <div className="absolute -mt-6 md:-mt-24 mr-14 -top-6 sm:top-6 right-8 sm:right-6 md:right-16 z-30">
          <GHStamp />
        </div>
      </div>
    </footer>
