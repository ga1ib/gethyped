...
function CardItem({ card, index, scrollYProgress }) {
  ...
  return (
    ...
    >
      <motion.div
        layout={false}
        className={`${card.bg} rounded-[2.5rem] w-full relative overflow-hidden md:overflow-visible shadow-xl`}
        style={{
          y,
          scale,
          opacity,
      ...
      >
        {/* ─────────── MOBILE LAYOUT (< md) ─────────── */}
        <div className="md:hidden relative z-10 flex flex-col h-full p-6">
          {/* Top: Expertise badge + watermark number aligned */}
          <div className="flex items-start justify-between">
            <span className="inline-block bg-white text-black text-sm font-semibold px-4 py-1.5 rounded-lg shadow-sm">
              Expertise
            </span>
            <span
              className={`font-black leading-none select-none pointer-events-none ${card.numColor}`}
          ...
          {/* Title under badge */}
          <motion.h2
            className="mt-3 font-black text-black leading-none"
          ...
          {/* Middle: video centered vertically, left aligned */}
          ...
          <div>
            ...
            <motion.p
              className="mt-2 text-black/65 text-base leading-relaxed"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
